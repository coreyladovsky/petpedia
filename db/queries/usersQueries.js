...
const getAllPetsForAUser = (req, res, next) => {
  let userId = parseInt(req.params.id);
  db.any('SELECT pets.* FROM users JOIN pets ON pets.owner_id = users.id WHERE users.id =$1', [userId])
    .then(data => {
      res.status(200)
        .json({
          status: 'success',
...
module.exports = { getAllUsers, getSingleUser, getAllPetsForAUser, updateUser, deleteUser, createUser}
